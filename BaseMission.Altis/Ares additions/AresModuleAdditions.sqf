...
["AI Behaviours", "Land Helicopter", {
    _veh = _this select 1;
    if !(_veh isKindOf "Air") exitWith {
        [objNull, "This isn't a air unit"] call BIS_fnc_showCuratorFeedbackMessage;
    };
    _veh land "LAND";
...
["Reinforcements", "Spawn Units v2", {
    ...
    if ((_pool select 11) isEqualTo []) exitWith {[objNull, "No infantry squads defined for side."] call BIS_fnc_showCuratorFeedbackMessage;};
    _vehiclePoolIndex = _dialogVehicleClass + 3;

    if ((_pool select _vehiclePoolIndex) isEqualTo []) exitWith {[objNull, "Vehicle pool had no vehicles defined for this faction."] call BIS_fnc_showCuratorFeedbackMessage;};

    _vehNames = [];
    _vehPool = []; {_vehName = getText (configFile >> "CfgVehicles" >> _x >> "displayname");_vehNames pushBack _vehName;_vehPool pushBack _x;} forEach (_pool select _vehiclePoolIndex);
    ...
    if (_vehiclePoolIndex == 7 || {_vehiclePoolIndex == 8}) then { {_x allowFleeing 0;} forEach (crew (vehicle (leader _vehicleGroup)));
    ...
    if (_dialogVehicleBehaviour == 2) then {
        ...
        if (count _dialog3Result < 1) exitWith {
            deleteVehicle (vehicle (leader _vehicleGroup)); {deleteVehicle _x} forEach (units (leader _vehicleGroup));
        ...
        private ["_delay", "_waypoint"];
        private _moveClockwise = (_dialog3Result select 1) == 0;
        ...
        private _centerPoint = position _lz;
        ...
        if (count _allRps > 0) then {
            ...
            switch (_dialogRpAlgorithm) do {
                ...
                case 3: {// Least Used
                    ...
                    {
                        _rp = _x;
                    } forEach (_allRps select {_x getVariable ["Ares_Rp_Count", 0] < _rp getVariable ["Ares_Rp_Count", 0]});
                };
                default {
...
["Reinforcements", "Spawn Supply Drop", {
    ...
    _dialogResult params ["_dialogPool", "_dialogVehicleClass", "_dialogLzAlgorithm"];
    _spawnPosition = _this select 0;
    ...
    switch (_dialogLzAlgorithm) do {
        ...
        case 3: {
            ...
            {
                _lz = _x;
            } forEach (_allLzs select {_x getVariable ["Ares_Lz_Count", 0] < _lz getVariable ["Ares_Lz_Count", 0]});
        };
        default {_lz = _allLzs select (_dialogLzAlgorithm - 4);};
    };
    ...
    _dialog2Result params ["_dialog2Type", "_dialog2Cargo"];
