...
cop_blue setPosASL (pos_cop_blue select 2); 
cop_blue setVectorDirAndUp [pos_cop_blue select 3, pos_cop_blue select 4];
...
menu_ground setPosASL (pos_menu_ground select 2); 
menu_ground setVectorDirAndUp [pos_menu_ground select 3, pos_menu_ground select 4];
...
ground_check setPosASL (pos_ground_check select 2); 
ground_check setVectorDirAndUp [pos_ground_check select 3, pos_ground_check select 4];
publicVariable "ground_check";

arsenal_1 = createVehicle [pos_arsenal_1 select 0, pos_arsenal_1 select 1, [], 0, 'CAN_COLLIDE']; 
arsenal_1 setPosASL (pos_arsenal_1 select 2); 
arsenal_1 setVectorDirAndUp [pos_arsenal_1 select 3, pos_arsenal_1 select 4];
[[arsenal_1], "scripts\box_virtualarsenal.sqf"] remoteExec ["execVM", 0, true];
[arsenal_1, ["Clear Loadout", "scripts\clear_loadout.sqf", [], 6, true, true, "", "_this distance _target < 5"]] remoteExec ["addAction", 0, true];
publicVariable "arsenal_1";

equipment_1 = createVehicle [pos_equipment_1 select 0, pos_equipment_1 select 1, [], 0, 'CAN_COLLIDE']; 
equipment_1 setPosASL (pos_equipment_1 select 2); 
equipment_1 setVectorDirAndUp [pos_equipment_1 select 3, pos_equipment_1 select 4];
...
_newObject setPosASL (_x select 2); 
_newObject setVectorDirAndUp [_x select 3, _x select 4];
